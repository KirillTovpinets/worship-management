...
interface SingerSongFiltersProps {
...
export default function SingerSongFilters({
  filters,
  onFiltersChange,
  ...
  const [mySongs, setMySongs] = useState<boolean | undefined>(undefined);
          ...
          {hasActiveFilters && (
            <span className="bg-red-500 text-white text-xs rounded-full px-2 py-1">
              {selectedTones.length +
                ...
                (search ? 1 : 0) +
                (hasEvents !== undefined ? 1 : 0) +
                (mySongs !== undefined ? 1 : 0)}
            </span>
          )}
        </button>
      </div>
              ...
              <div className="space-y-2">
                <label className="flex items-center">
                  <input
                    type="radio"
                    name="mySongs"
                    checked={mySongs === true}
                    onChange={() => setMySongs(true)}
                    className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="ml-2 text-sm text-gray-700">
                    Show only songs in my key
                  </span>
                </label>
                <label className="flex items-center">
                  <input
                    type="radio"
                    name="mySongs"
                    checked={mySongs === false}
                    onChange={() => setMySongs(false)}
                    className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="ml-2 text-sm text-gray-700">
                    Show all songs
                ...
                <label className="flex items-center">
                  <input
                    type="radio"
                    name="mySongs"
                    checked={mySongs === undefined}
                    onChange={() => setMySongs(undefined)}
                    className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                  />
                  <span className="ml-2 text-sm text-gray-700">
                    No preference
